...
1. **Filtra** cada señal con la prueba de sustancia de abajo. Las que no la pasen van a
   `descartes`, una por una, con su código.
2. **Agrupa** las que sí la pasan por *frente de intervención*: un programa, una obra, un
   sistema. No por contrato — varios contratos suelen pagar el mismo frente.
3. **Escribe un insight por grupo**, nunca uno por contrato.

Un municipio de este tamaño rara vez tiene más de cinco o seis frentes en una quincena. **Si
produces más insights que eso, no agrupaste: vuelve al paso 2 y fusiona.**
...
**`cambio_fisico` no fuerza a separar.** Si un mismo frente deja varias cosas sobre el suelo,
enuméralas todas en un solo insight: "pavimento nuevo en la carrera 73, la calle 80 y las calles
81 y 82". Un frente de pavimentación con cuatro contratos es **un** insight con cuatro
evidencias, no cuatro insights.
...
Elige **el código más específico** que aplique, no el más general. `servicios_profesionales` es
el más general de todos: úsalo solo cuando ningún otro encaje. Si un contrato de suministro lo
firma un profesional, el código es `suministro`.

| Código | Cuándo |
|---|---|
| `suministro` | Compra de bienes, dotación, repuestos, papelería, alimentos, combustible |
| `mantenimiento_rutinario` | Reparar, calibrar, conservar, sin ampliar capacidad |
| `evento_capacitacion` | Formación, campañas, sensibilización, eventos, publicidad |
| `estudio_diagnostico` | Estudios y consultorías sin obra identificable asociada |
| `personal` | Nómina, vinculación laboral, seguridad social |
| `servicios_profesionales` | Asesoría o apoyo a la gestión, cuando ninguno de los anteriores aplica |
| `sin_implicacion_inmobiliaria` | No toca el territorio y no encaja arriba |
