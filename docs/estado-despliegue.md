...
**Actualizado:** 2026-09-23 · rama `remediacion/f0`, 26 commits por delante de
`main`

> **Para retomar:** lee esto primero, después
> [decisiones-remediacion.md](decisiones-remediacion.md).

## Progreso: 2 de 11 pasos

| | Paso | Estado |
|---|---|---|
| 1 | **Revisión de Vercel y compatibilidad de migraciones** | ✅ **Cerrado** |
| 2 | **Revisión de `remediacion/f0` frente a `main`** | ✅ **Cerrado** |
| 3 | **Push de la rama** a `origin` (sin PR) | ⬜ **Siguiente** |
| 4 | **Variables en Vercel**: solo *Production*, base `territorial` con **pooler**, `COOKIE_SECRET`, raíz `web` | ⬜ |
| 5 | `pg_dump` de la base principal, fuera del repositorio | ⬜ |
...
- **Nunca publicar desde `main` después de migrar.** Su `publicar()` no escribe
  ...
  `remediacion/f0`.
- **Los pasos 5 a 8 van en una misma sesión.** Entre migrar y republicar el
