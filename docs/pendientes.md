...
| **B5** | **El Clasificador se trunca con lotes grandes.** Con 40 señales de Barranquilla produjo 6 insights y 31 descartes, y el JSON llegó cortado: `Invalid JSON: EOF while parsing a string`. El techo de 4096 tokens de salida no da para un descarte por señal. Hasta que se suba, los municipios de alto volumen no se pueden procesar en lotes grandes | M2 sobre las 6 ciudades de mayor volumen | Área de Analítica | Prueba de M4, 2026-09-17 |
| **B6** | **M2 no persiste nada.** Hay 0 insights en la base: el Clasificador solo ha corrido en scripts de prueba. Sin persistencia no hay entrada estable para M4, no hay qué calificar en M7 y F6 del scoring nunca tendrá datos | M4, M5 (F6), M7 — el ciclo completo | Área de Analítica | Prueba de M4, 2026-09-17 |
